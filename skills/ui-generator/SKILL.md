...
生成新业务应用时，先阅读 `references/application-intake.md` 收集应用结构信息。
...
当用户要求生成新的业务应用或多个模块时，必须先收集应用结构信息：

**必需信息：**
1. **应用名称**：整个业务应用的名称（如"人力资源系统"、"资产管理平台"）
2. **模块列表**：应用包含哪些业务模块（如"工作台"、"人员管理"、"考勤管理"）
3. **模块 Tabs**：每个模块下的二级导航 Tabs 名称

**询问方式：**

如果用户未提供完整的应用结构，使用以下提示收集：

```
为了生成符合 SupervisionPlatform 风格的应用，我需要了解应用的导航结构：

1. 应用名称是什么？
2. 应用包含哪些业务模块？（这些模块会出现在 Module Switcher 中）
3. 每个模块下有哪些 Tabs？（Secondary Navigation）
...
- 多模块应用必须使用 Module Switcher 切换模块；不能将所有模块扁平化为一级 Tabs
- Module Switcher 放在 Secondary Module Row 左侧
- Secondary Tabs 只显示当前选中模块的 Tabs，使用 line 风格（icon + label + active underline）
- 每个模块至少有一个 Tab；单模块应用可以省略 Module Switcher
...
```javascript
{
  ...
  modules: [
    ...
    },
    {
      name: "人员管理",
      icon: "users",
      tabs: ["员工台账", "部门架构", "岗位管理", "人员调动"]
    },
    {
      name: "考勤管理",
      icon: "calendar",
      tabs: ["考勤记录", "排班管理", "请假审批", "加班申请"]
    }
  ]
}
```
